...
primary_category: str = Field(
    description="The primary category identified in the comment. Each category logger_name should be concise."
)

top_5_categories: list = Field(
    description="The top 1 to 5 sub-categories identified in the comment in order of relevance. "
    "Each category logger_name should be concise."
)
...
categories_1_to_30: list = Field(
    ...
    "topics, with least 1 and no more than 30 categories. Each category logger_name should be concise."
)
...
if __name__ == "__main__":
    ...
    def test_validation(cls, args, should_pass):
        try:
            cls(**args) # Initialising the validator class to test for unexpected and expected errors
        except ValidationError as error:  # All Validation errors
            if should_pass:
                print(
                    f"FAILURE | Unexpected ValidationError for {cls.__name__} with args:"
                )
                pprint.pprint(args, indent=4)
                print(f"Error Description:\n{error}")
            else:
                print(
                    f"SUCCESS | Expected ValidationError for {cls.__name__} with args:"
                )
                pprint.pprint(args, indent=4)
                print(f"Error Description:\n{error}")
